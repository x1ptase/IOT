...
\begin{document}
	...
	\begin{table}[h]
		\centering
		\caption{Testing Results Summary} % Bạn nên thay đổi tên caption này
		...
		\begin{tabular}{p{3cm} p{2cm} p{4.5cm} p{4.5cm} c}
			...
			RTC Accuracy & DS1307 & Time deviation $< \SI{1}{\minute}$ per month & $\approx \SI{1}{\minute}$ time difference per month & Success \\
			\addlinespace
			
			Temperature Reading & LM35 & Reading within \SI{0.5}{\degreeCelsius} of calibrated thermometer & Reading within \SI{0.5}{\degreeCelsius} of calibrated thermometer & Success \\
			\addlinespace
			
			Wireless Command Latency & HC-05 \& Arduino & Message update latency $< \SI{1}{\second}$ & Message update latency $> \SI{4}{\second}$ & Success \\
			\addlinespace
			
			EEPROM Persistence & EEPROM & Retention of Mode/Message after \SI{1}{\minute} power-off & Retention of Mode/Message after \SI{1}{\minute} power-off & Success \\
			\addlinespace
			
			Mode \& Effect Cycling & Arduino & Successful execution of all 5 Modes and 5 Scroll Effects & Successful execution of all 5 Modes and 5 Scroll Effects & Success \\
