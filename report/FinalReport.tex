...
\begin{document}
	...
	Initial functional checks were conducted as shown in Table \ref{tab:testing_results}.
...
\begin{table}[h]
	...
	\begin{tabular}{p{3cm} p{2cm} p{4.5cm} p{4.5cm} c}
		...
		RTC Accuracy & DS1307 & $\text{Deviation } < \SI{1}{\minute} / \text{month}$ & $\approx \SI{1}{\minute} / \text{month}$ & Success \\
		\addlinespace
		...
		Response Time & HC-05 \& Arduino & Message update latency must be less than \SI{1}{\second}. & Message update latency observed to be less than \SI{1}{\second}. & Success \\
		\addlinespace
		
		EEPROM Persistence & EEPROM & Configuration maintenance after \SI{1}{\minute} power-off. & Configuration successfully retained after \SI{1}{\minute} power-off. & Success \\
		\addlinespace
...
\end{table}
	...
	\begin{thebibliography}{9}
		...
		\textit{Arduino UNO Rev3 - Technical Specifications}.
